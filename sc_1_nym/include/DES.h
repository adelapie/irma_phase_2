...
#define PRIM_BLOCK_ENCIPHER_ECB 0xDB

#define multosBlockEncipherCBC(algorithm, inputLength, plainText, cipherText, initialValueLength, initialValue, keyLength, key) \
do { \
  __push (__typechk(unsigned char, initialValueLength)); \
  __push (__typechk(unsigned char *, initialValue)); \
  __push (__typechk(unsigned int, inputLength)); \
  __push (__typechk(unsigned char *, key)); \
  __push (__typechk(unsigned char, keyLength)); \
  __push (__typechk(unsigned char *, cipherText)); \
  __push (__typechk(unsigned char *, plainText)); \
  __code (PRIM, PRIM_BLOCK_ENCIPHER_ECB, algorithm, 2); \
  } while (0)
...
do { \
  ...
  __push (__typechk(unsigned char *, plainText)); \
  __push (__typechk(unsigned char *, cipherText)); \
  __code (PRIM, PRIM_BLOCK_DECIPHER_ECB, algorithm, 2); \
} while (0)
