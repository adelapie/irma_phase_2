...
unsigned char alpha_long[SIZE_M] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x03};
unsigned char challenge_extended[64];
                                        
unsigned char result_2[SIZE_M*4];
unsigned char result_5[SIZE_M_];

unsigned char minus_alpha_long[SIZE_M] = 
    ...
    switch (INS) {
      ...
      case INS_PROVE_ATTRIBUTE:
        ...
        if (P2 == 0x23) {
          ...
          /* Two's complement multiplication with sign extension */

          Copy(SIZE_M, session.prove.var1, minus_alpha_long);
          Copy(SIZE_M, session.prove.var2, r);
          multosBlockMultiplyExtended(SIZE_M, session.prove.var1, session.prove.var2, session.prove.var3);
          Fill(SIZE_M, session.prove.var3, 0xff);

          Clear(SIZE_M*2, challenge_extended);
          Copy(SIZE_M, challenge_extended + SIZE_M, session.prove.challenge);
          multosBlockMultiplyExtended(SIZE_M*2, session.prove.var3, challenge_extended, public.prove.buffer.number[0]);
                    
          Copy(SIZE_M*2, session.prove.var3, public.prove.buffer.number[0] + SIZE_M*2);
          Fill(SIZE_M_, public.prove.buffer.number[1], 0xff);

          Copy(SIZE_M*2, public.prove.buffer.number[1] + (SIZE_M_ - SIZE_M*2), session.prove.var3);
          
          multosBlockAdd(SIZE_M_, session.prove.mHatTemp, public.prove.buffer.number[1], public.apdu.data);
