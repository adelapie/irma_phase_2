...
def proveCommitmentNYM_ONLY(connection, pk_i, CRED_ID, DEBUG, Rdom, Rr, r1): 
  ...
  input = {'pChat':c, 'n3':irma_util.APDU2integer(NONCE), 'pAprime':a, 'pEhat':e, 'pVprimeHat':v, 'mHatMs':ms, 'NYM1':ESTA33, 'NYM2':NYM_1}
  m = { '1':m1, '2':m2, '3':m3, '4':m4, '5':m5 }
    
  verifier = protocol_ibm12.Verifier(pk_i, irma_util.APDU2integer(CONTEXT))
  return (data, sw1, sw2, verifier.verifyAllIRMA_NYM_ONLY(m, input))

def proveCommitmentNYM_ONLY_H(connection, pk_i, CRED_ID, DEBUG, Rdom, Rr, r1): 
  ...
  verifier = protocol_ibm12.Verifier(pk_i, irma_util.APDU2integer(CONTEXT))
  return (data, sw1, sw2, verifier.verifyAllIRMA_NYM_H_ONLY(m, input))
