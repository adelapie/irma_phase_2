...
extern unsigned char RSA_pt[RSA_MOD_BYTES];
...
/*void authentication_generateChallenge(RSA_public_key *key, unsigned char *challenge) { 
  RandomBytes(&session.prove.nonceC[0], 1);
  RandomBytes(session.prove.nonceC, SIZE_STATZK);

  Fill(RSA_MOD_BYTES, RSA_pt, 0x00);

  Copy(SIZE_STATZK, RSA_pt, session.prove.nonceC);
  RSA_RAW_encrypt(challenge, key, RSA_MOD_BYTES, RSA_pt);
}
*/
