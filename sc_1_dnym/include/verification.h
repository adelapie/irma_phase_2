...
do { \
  __push(__typechk(unsigned char *, domainIn)); \
  __push(__typechk(unsigned char *, pubKeyIn)); \
  __push(__typechk(unsigned char *, sigIn)); \
  __push(__typechk(unsigned char *, hashIn)); \
  __code (PRIM, PRIM_VER_ECC_SIG, option); \
  __code (PRIM, PRIM_LOAD_CCR); \
  __code (PRIM, PRIM_BIT_MANIPULATE_BYTE, (0x80), 0x01); \
  __code (STORE, __typechk(unsigned char *, success), 1); \
} while (0)

#define multosPad(msgLen, msg, blockLen, paddedMsgLen, padMethod) \
do { \
  __push (__typechk(unsigned char *, blockLen)); \
  __push (__typechk(unsigned short, msgLen)); \
  __push (__typechk(unsigned char *, msg)); \
  __code (PRIM, PRIM_PAD, padMethod); \
  __code (STORE, __typechk(unsigned short *, paddedMsgLen), 2); \
} while (0)
