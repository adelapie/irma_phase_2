...
void authentication_generateChallenge(RSA_public_key *key, unsigned char *challenge) { 
  ...
  Fill(RSA_MOD_BYTES, public.apdu.session, 0x00);

  Copy(SIZE_STATZK, public.apdu.session, session.prove.nonceC);

  RSA_RAW_encrypt(challenge, key, RSA_MOD_BYTES, public.apdu.session);
}
