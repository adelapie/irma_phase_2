...
//  unsigned char p_exp[3] = {0x01,0x00,0x01};
...
//  ModExp(RSA_EXP_BYTES, RSA_MOD_BYTES, key->exponent, public.apdu.data + SIZE_STATZK, plaintext, ciphertext);

//ok    ModExp(3, RSA_MOD_BYTES, p_exp, public.apdu.data + SIZE_STATZK, plaintext, ciphertext);

    ModExp(3, RSA_MOD_BYTES, public.apdu.data + SIZE_STATZK + RSA_MOD_BYTES, public.apdu.data + SIZE_STATZK, plaintext, ciphertext);

//  Fill(RSA_MOD_BYTES, ciphertext, 0xaa);
